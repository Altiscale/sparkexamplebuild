...
if [ "x${DEBUG_MAVEN}" = "xtrue" ] ; then
  mvn_cmd="mvn -U -X package -Pspark-2.3 -Pkafka10-provided $testcase_hadoop_profile_str"
else
  mvn_cmd="mvn -U package -Pspark-2.3 -Pkafka10-provided $testcase_hadoop_profile_str"
fi
