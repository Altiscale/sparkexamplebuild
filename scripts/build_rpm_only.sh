...
spark_git_dir=$WORKSPACE/sparkexample
spark_spec="$curr_dir/sparkexample.spec"
...
# sed -i 's/git\@github.com:Altiscale\/sparkexample.git/https:\/\/github.com\/Altiscale\/sparkexample.git/g' .gitmodules
# sed -i 's/git\@github.com:Altiscale\/sparkexample.git/https:\/\/github.com\/Altiscale\/sparkexample.git/g' .git/config
...
cp "$spark_spec" $WORKSPACE/rpmbuild/SPECS/$(basename $spark_spec)

pushd $WORKSPACE/
tar --exclude .git --exclude .gitignore -cf $WORKSPACE/rpmbuild/SOURCES/sparkexample.tar sparkexample
popd
...
if [ -f "$maven_settings" ] ; then
  ...
  rpmbuild -vv -ba $WORKSPACE/rpmbuild/SPECS/$(basename $maven_settings_spec) \
...
rpmbuild -vv \
  -ba $WORKSPACE/rpmbuild/SPECS/$(basename $spark_spec) \
  --define "_topdir $WORKSPACE/rpmbuild" \
  ...
  --define "_apache_name $SPARKEXAMPLE_PKG_NAME" \
  --define "_build_release $BUILD_TIME" \
  --define "_production_release $PRODUCTION_RELEASE" \
  --buildroot $WORKSPACE/rpmbuild/BUILDROOT/

if [ $? -ne "0" ] ; then
  echo "fail - sparkexample SRPM build failed"
  cleanup_secrets
  exit -98
fi

stat "$WORKSPACE/rpmbuild/SRPMS/alti-spark-${SPARK_VERSION}-example-${SPARK_VERSION}-${ALTISCALE_RELEASE}.${BUILD_TIME}.el6.src.rpm"
rpm -ivvv "$WORKSPACE/rpmbuild/SRPMS/alti-spark-${SPARK_VERSION}-example-${SPARK_VERSION}-${ALTISCALE_RELEASE}.${BUILD_TIME}.el6.src.rpm"
...
rpmbuild -vv --bi $WORKSPACE/rpmbuild/SPECS/$(basename $spark_spec) --short-circuit --buildroot $WORKSPACE/rpmbuild/BUILDROOT/ \
  --define "_topdir $WORKSPACE/rpmbuild" \
  --define "_current_workspace $WORKSPACE" \
  --define "_spark_version $SPARK_VERSION" \
  --define "_scala_build_version $SCALA_VERSION" \
